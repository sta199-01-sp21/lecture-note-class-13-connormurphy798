...
I probably would not be surprised if 3 were left handed, but I would be quite
surprised if 40 were.
...
```{r data_manhattan}
manhattan <- read_csv("~/R/STA199/lc/lc13/manhattan.csv")
```
...
**Interpretation**: This means that, if many samples were taken, 95% percent of
them would contain the true mean value.
...
- Population: mice that are not on treatment
...
**Interpretation**: This means we are 95% confident that the true mean lies
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop_2, level = 0.90)
ci_mice_lived
```
...
We can be 95% percent confident that the true proportion of mice that lived
while on the treatment will fall within the interval described by 
`ci_mice_lived`.
